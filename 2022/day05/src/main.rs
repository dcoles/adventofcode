...
fn main() {
    let input = Input::from_file("day05/input.txt").expect("failed to read input");
...
fn part2(input: &Input) -> String {
    ...
    for step in &input.steps {
        ...
        for _ in 0..step.count {
            crane.push(crates[step.from - 1].pop().expect("tried to move missing crate"));
        }

        for _ in 0..step.count {
            crates[step.to - 1].push(crane.pop().unwrap());
        }
    }
